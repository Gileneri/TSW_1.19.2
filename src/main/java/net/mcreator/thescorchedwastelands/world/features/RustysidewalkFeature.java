...
public class RustysidewalkFeature extends Feature<NoneFeatureConfiguration> {
	public static RustysidewalkFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new RustysidewalkFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("the_scorched_wastelands:rustysidewalk", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("the_scorched_wastelands:rustysidewalk", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("the_scorched_wastelands", "rusty_sidewalk1"));
		if (template == null)
			return false;
